...
;;;
;;; Источник геометрии:
;;;   LWPOLYLINE — замкнутая полилиния, площадь по AutoCAD Area,
;;;                 габариты по BoundingBox;
;;;   INSERT      — динамический блок: ширина/высота из динамических
;;;                 свойств, при отсутствии — BoundingBox.
;;;
;;; Наработки CLADDING:
;;;   - EffectiveName / Visibility;
;;;   - безопасное чтение динамических свойств;
;;;   - фактическая площадь LWPOLYLINE;
;;;   - контроль дуг/подрезных блоков;
;;;   - фильтрация по слоям и предвыбору.
;;;
;;; Раскрой:
;;;   MaxRects / Best Short Side Fit, вращение 0/90°;
;;;   пропил моделируется как дополнительный зазор справа и сверху;
;;;   детали сортируются по габариту и площади;
;;;   не помещающиеся детали выдаются отдельно.
;;;
;;; Вывод:
;;;   - нативная графическая карта AutoCAD;
;;;   - XLS (Excel XML без COM), при отказе — CSV;
;;;   - сводка по листам и позициям;
;;;   - единый блок карты раскроя;
;;;   - рамка на слое "Невидимые".
;;;
;;; Совместимость с dispatcher:
;;;   (cutsheet-main layers)
;;;   используются *CUTSHEET-CREATE-XLS* и *CUTSHEET-CREATE-TABLE*.
;;; ============================================================
...
;; Состояние диалога
(if (not (boundp '*cs-tmp-choice*))
  (setq *cs-tmp-choice* 'ALL))
(if (not (boundp '*cs-tmp-sheet-w*))
  (setq *cs-tmp-sheet-w* *CUTSHEET-DEFAULT-WIDTH*))
(if (not (boundp '*cs-tmp-sheet-h*))
  (setq *cs-tmp-sheet-h* *CUTSHEET-DEFAULT-HEIGHT*))
(if (not (boundp '*cs-tmp-kerf*))
  (setq *cs-tmp-kerf* *CUTSHEET-DEFAULT-KERF*))
(if (not (boundp '*cs-tmp-rotate*))
  (setq *cs-tmp-rotate* *CUTSHEET-DEFAULT-ROTATE*))
(if (not (boundp '*cs-tmp-xls*))
  (setq *cs-tmp-xls* T))
(if (not (boundp '*cs-tmp-acad*))
  (setq *cs-tmp-acad* T))
(if (not (boundp '*cs-tmp-dyn-type*))
  (setq *cs-tmp-dyn-type* ""))
(if (not (boundp '*cs-dyn-types*))
  (setq *cs-dyn-types* '()))
(if (not (boundp '*CUTSHEET-LAST-WIDTH*))
  (setq *CUTSHEET-LAST-WIDTH* *CUTSHEET-DEFAULT-WIDTH*))
(if (not (boundp '*CUTSHEET-LAST-HEIGHT*))
  (setq *CUTSHEET-LAST-HEIGHT* *CUTSHEET-DEFAULT-HEIGHT*))
(if (not (boundp '*CUTSHEET-LAST-KERF*))
  (setq *CUTSHEET-LAST-KERF* *CUTSHEET-DEFAULT-KERF*))
(if (not (boundp '*CUTSHEET-LAST-ROTATE*))
  (setq *CUTSHEET-LAST-ROTATE* *CUTSHEET-DEFAULT-ROTATE*))
(if (not (boundp '*CUTSHEET-LAST-XLS*))
  (setq *CUTSHEET-LAST-XLS* T))
(if (not (boundp '*CUTSHEET-LAST-ACAD*))
  (setq *CUTSHEET-LAST-ACAD* T))
...
(defun cs-block-all-props (obj / dyn prop pname pval out r)
  (setq out '())
  (setq dyn (vl-catch-all-apply 'vlax-invoke
...
(defun cs-prop-value (props wanted / p)
  ...
  (foreach x props
    (if (and (null p)
             (= (strcase (car x)) (strcase wanted)))
      (setq p (cdr x))))
  (if p (cs-value-to-number p) nil)
)

(defun cs-prop-string (props names / found p name)
  (setq found nil)
  (foreach p props
    (if (null found)
      (progn
        (setq name (strcase (car p)))
        (foreach wanted names
          (if (and (null found)
                   (vl-string-search (strcase wanted) name))
            (setq found (vl-princ-to-string (cdr p))))))))
  (if found (vl-string-trim " \t\r\n" found) nil)
)
...
(defun cs-bbox-w-h (ent / bb p1 p2)
  ...
  (if bb
    (progn
      ...
      (list (abs (- (car p2) (car p1)))
            (abs (- (cadr p2) (cadr p1))))
    )
  )
)

;; Площадь подрезного динамического блока — та же модель,
;; которая используется в Облицовке: Б=Высота, С=Ширина,
;; углы D/E/F/G. При отсутствии углов площадь = B*C.
(defun cs-block-area-from-props (props w h / d e f g a)
  (setq d (or (cs-prop-value props "Правый угол") 0.0)
        e (or (cs-prop-value props "Левый угол") 0.0)
        f (or (cs-prop-value props "Верхний угол") 0.0)
        g (or (cs-prop-value props "Нижний угол") 0.0))
  (if (and (= d 0.0) (= e 0.0) (= f 0.0) (= g 0.0))
    (/ (* w h) 1000000.0)
    (progn
      ;; w = ширина C, h = высота B
      (setq a (- (* w h)
                 (* (- w d) g)
                 (* (- w d) (- h f))
                 (* e g)
...
(defun cs-poly-has-arcs (ent / found)
  ...
  (foreach g (entget ent)
    (if (and (= (car g) 42) (> (abs (cdr g)) 1e-8))
      (setq found T)))
  found)
...
(defun cs-layer-ok-p (layer layers)
  (if (or (null layers) (= (length layers) 0))
    T
    (su-layer-match-any layer layers)))

(defun cs-build-filter-ss (layers / ss out i ent typ lay)
  ;; 1) Предвыбор из диспетчера/AutoCAD
  (setq ss nil)
  ...
  ;; 2) Если предвыбора нет — чертим набор вручную/по слоям
  (if (null ss)
    (if (and layers (> (length layers) 0))
      (progn
        (setq ss (ssget "_X"
                 (list '(0 . "LWPOLYLINE,INSERT")
                       (cons 8
                         (apply 'strcat
  ...
  ;; 3) Дополнительная фильтрация масок слоя
  (setq out (ssadd) i 0)
  (if ss
    (repeat (sslength ss)
      ...
      (if (and (or (= typ "LWPOLYLINE") (= typ "INSERT"))
               (cs-layer-ok-p lay layers))
        (ssadd ent out))
      (setq i (1+ i))))
...
;; ================= ЗАПИСЬ ЧАСТЕЙ =================
;; record:
;; (id kind layer type width height area nominal exact? ent)
  ...
  (if (and (cs-poly-closed-p ent) bb)
    (progn
      (setq wh (cs-bbox-w-h ent)
            area (cs-poly-area ent)
            arc (cs-poly-has-arcs ent))
      (if (and wh (> (car wh) 0.0) (> (cadr wh) 0.0)
               (> area 0.0))
        (progn
...
(defun cs-block-record (ent id / obj ed layer props typName wh w h
                             pW pH area cut nominal source)
  (setq ed (entget ent)
        layer (cdr (assoc 8 ed))
  ...
  (if (vl-catch-all-error-p obj)
    nil
    (progn
      (setq props (cs-block-all-props obj)
            typName (cs-get-dyn-type-name ent))

      ;; Основной путь — динамические свойства "Ширина" / "Высота".
      (setq pW (cs-prop-value props "Ширина")
            pH (cs-prop-value props "Высота"))

      ;; Резервный путь — габарит блока в AutoCAD.
      ...
      (if (and w h (> w 0.0) (> h 0.0))
        (progn
          (setq area (if (and pW pH)
                       (cs-block-area-from-props props w h)
                       (/ (* w h) 1000000.0)))
          (setq cut
            (or (> (or (cs-prop-value props "Правый угол") 0.0) 0.0)
                (> (or (cs-prop-value props "Левый угол") 0.0) 0.0)
                (> (or (cs-prop-value props "Верхний угол") 0.0) 0.0)
                (> (or (cs-prop-value props "Нижний угол") 0.0) 0.0)))
          (setq nominal (strcat (cs-itoa-safe w) "x" (cs-itoa-safe h)))
          (list id "DYN" layer typName w h area nominal (if cut "ПОДРЕЗ" source) ent))
        nil))))
...
(defun cs-collect-records (ss choice dynType / i ent typ rec out id)
  ...
  (if ss
    (repeat (sslength ss)
      ...
      (cond
        ((and (= typ "LWPOLYLINE")
              (or (eq choice 'ALL) (eq choice 'POLY)))
         (setq id (1+ id))
         (setq rec (cs-poly-record ent id)))
        ((and (= typ "INSERT")
              (or (eq choice 'ALL) (eq choice 'DYN)))
         (if (or (= dynType "") (null dynType)
                 (= dynType "Все типы блоков"))
           (progn
             (setq id (1+ id))
             (setq rec (cs-block-record ent id)))
           (if (= (cs-get-dyn-type-name ent) dynType)
             (progn
               (setq id (1+ id))
               (setq rec (cs-block-record ent id)))))
        )
  ...
  (if ss
    (repeat (sslength ss)
      (setq ent (ssname ss i)
            typ (cdr (assoc 0 (entget ent))))
      (cond
        ((and (= kind 'POLY) (= typ "LWPOLYLINE"))
         (if (cs-poly-closed-p ent) (setq cnt (1+ cnt))))
        ((and (= kind 'DYN) (= typ "INSERT"))
         (setq obj (vl-catch-all-apply 'vlax-ename->vla-object (list ent)))
         (if (not (vl-catch-all-error-p obj))
           (progn
             ...
             (if (and wh (> (car wh) 0.0) (> (cadr wh) 0.0))
               (setq cnt (1+ cnt))))))
        ((eq kind 'ALL)
         (if (or (= typ "LWPOLYLINE") (= typ "INSERT"))
           (setq cnt (1+ cnt))))
      )
      (setq i (1+ i)))
  )
  ...
  (if ss
    (repeat (sslength ss)
      (setq ent (ssname ss i)
            typ (cdr (assoc 0 (entget ent))))
      (if (and (= typ "INSERT")
               (or (= wanted "") (= (cs-get-dyn-type-name ent) wanted)))
        (setq cnt (1+ cnt)))
      (setq i (1+ i)))
  )
  cnt)
...
;; ================= DCL =================
(defun cs-safe-set-tile (key val)
  (vl-catch-all-apply 'set_tile (list key val)))

(defun cs-cut-sizes-valid-p (w h kerf)
  (and (numberp w) (numberp h) (> w *CUTSHEET-MIN-SIZE*)
       (> h *CUTSHEET-MIN-SIZE*)
       (< w *CUTSHEET-MAX-SIZE*) (< h *CUTSHEET-MAX-SIZE*)
       (numberp kerf) (>= kerf 0.0)
       (< kerf (min w h))))

(defun cs-dialog
       (polyCnt dynCnt dynTypes ss
        defaultW defaultH defaultKerf defaultRotate
        defaultXls defaultAcad
        ...
        (progn
          ...
          (if (not (new_dialog "cutsheet_filter_dialog" dcl-id))
            ...
            (progn
              ...
              ;; Список типов динамических блоков.
              ...
              (if (<= dynCnt 0)
                (cs-safe-set-tile "popup_dyn_type" "0"))
              ...
              ;; Радиокнопки: callback только меняет состояние, не меняет UI.
              (action_tile "rb_all" "(setq *cs-tmp-choice* 'ALL)")
              (action_tile "rb_poly" "(setq *cs-tmp-choice* 'POLY)")
              (action_tile "rb_dyn" "(setq *cs-tmp-choice* 'DYN)")
              (action_tile "popup_dyn_type"
                "(if (= (atoi $value) 0) (setq *cs-tmp-dyn-type* \"\") (setq *cs-tmp-dyn-type* (nth (1- (atoi $value)) *cs-dyn-types*))) (setq *cs-tmp-choice* 'DYN)")
...
;; ================= АГРЕГАЦИЯ =================
(defun cs-part-key (r)
  (strcat (nth 1 r) "|" (nth 3 r) "|"
          (cs-itoa-safe (nth 4 r)) "x" (cs-itoa-safe (nth 5 r))))

(defun cs-part-label (r)
  (strcat (cs-itoa-safe (nth 4 r)) "x" (cs-itoa-safe (nth 5 r))))

(defun cs-aggregate (records / acc r key f out)
  ;; group: (key kind layer type W H count bboxArea factArea)
  (setq acc '())
  (foreach r records
    (setq key (cs-part-key r)
          f (assoc key acc))
    (if f
      (setq acc
        (subst
          (list key (nth 1 r) (nth 2 r) (nth 3 r)
                (nth 4 r) (nth 5 r)
                (1+ (nth 6 f))
                (+ (nth 7 f) (* (nth 4 r) (nth 5 r) (/ 1.0 1000000.0)))
                (+ (nth 8 f) (nth 6 r)))
          f acc))
      (setq acc
        (cons
...
;; ================= SORT / NESTING =================
;; Примечание: габаритные прямоугольники используются как консервативная
;; оболочка для LWPOLYLINE. Фактическая площадь хранится отдельно и не
;; подменяется площадью BoundingBox.
(defun cs-expanded-sorted (records / sorted out r n i)
  ;; max(W,H) -> area -> original id
  (setq sorted
    (vl-sort records
      '(lambda (a b)
         (cond
           ((> (max (nth 4 a) (nth 5 a))
               (max (nth 4 b) (nth 5 b))) T)
           ((< (max (nth 4 a) (nth 5 a))
               (max (nth 4 b) (nth 5 b))) nil)
           ((> (* (nth 4 a) (nth 5 a))
               (* (nth 4 b) (nth 5 b))) T)
           ((< (* (nth 4 a) (nth 5 a))
               (* (nth 4 b) (nth 5 b))) nil)
...
(defun cs-rect-intersects-p (a b)
  (and (< (car a) (+ (car b) (nth 2 b)))
       (< (car b) (+ (car a) (nth 2 a)))
       (< (cadr a) (+ (cadr b) (nth 3 b)))
       (< (cadr b) (+ (cadr a) (nth 3 a)))))

(defun cs-free-split-one (fr px py pw ph / fx fy fw fh fx2 fy2 px2 py2 out)
  (setq fx (nth 0 fr) fy (nth 1 fr)
        fw (nth 2 fr) fh (nth 3 fr)
        fx2 (+ fx fw) fy2 (+ fy fh)
        px2 (+ px pw) py2 (+ py ph)
        out '())
  (if (not (cs-rect-intersects-p fr (list px py pw ph)))
    (list fr)
    (progn
      ;; Лево
      (if (> (- px fx) 1e-8)
        (setq out (cons (list fx fy (- px fx) fh) out)))
      ;; Право
      (if (> (- fx2 px2) 1e-8)
        (setq out (cons (list px2 fy (- fx2 px2) fh) out)))
      ;; Низ
      (if (> (- py fy) 1e-8)
        (setq out (cons (list fx fy fw (- py fy)) out)))
      ;; Верх
      (if (> (- fy2 py2) 1e-8)
        (setq out (cons (list fx py2 fw (- fy2 py2)) out)))
      (reverse out))))

(defun cs-prune-free (free / out a b drop)
  ;; Удаление нулевых и вложенных прямоугольников.
  (setq out '())
  (foreach a free
    (if (and (> (nth 2 a) 1e-8) (> (nth 3 a) 1e-8))
      (setq out (cons a out))))
  (setq free (reverse out)
        out '())
  (foreach a free
    (setq drop nil)
    (foreach b free
...
(defun cs-find-placement-in-free
       (freeRects pw ph allowRotate / fr best bestScore score sw sh dx dy minL maxL)
  ;; Returns (score x y w h rot), NIL if no fit.
  ;; Пропил учитывается только если после детали действительно остаётся
  ;; место для соседней детали. В край листа пропил не добавляется.
  (foreach fr freeRects
    (setq sw pw
          sh ph)
    (if (<= (+ sw (if (> (- (nth 2 fr) sw) *CUTSHEET-WORK-KERF*)
                       *CUTSHEET-WORK-KERF* 0.0))
            (+ (nth 2 fr) 1e-8))
      (if (<= (+ sh (if (> (- (nth 3 fr) sh) *CUTSHEET-WORK-KERF*)
                         *CUTSHEET-WORK-KERF* 0.0))
              (+ (nth 3 fr) 1e-8))
        (progn
          (setq dx (- (nth 2 fr) sw)
                ...
                minL (min dx dy)
                maxL (max dx dy)
                score (list minL maxL 0))
          (if (or (null bestScore)
                  (< (car score) (car bestScore))
                  (and (= (car score) (car bestScore))
                       (< (cadr score) (cadr bestScore))))
            (setq bestScore score
                  best (list (car score) (nth 0 fr) (nth 1 fr) pw ph 0)))))))
        ...
        (if (<= (+ sh (if (> (- (nth 3 fr) sh) *CUTSHEET-WORK-KERF*)
          ...
          (progn
            (setq dx (- (nth 2 fr) sw)
                  ...
                  minL (min dx dy)
                  maxL (max dx dy)
                  score (list minL maxL 1))
            (if (or (null bestScore)
                    (< (car score) (car bestScore))
                    (and (= (car score) (car bestScore))
                         (< (cadr score) (cadr bestScore))))
              (setq bestScore score
                    best (list (car score) (nth 0 fr) (nth 1 fr) ph pw 1))))))))
  best)

(defun cs-add-placement (freeRects placement / newFree fr fx fy fw fh px py pw ph occW occH)
  (setq newFree '())
  (foreach fr freeRects
    (setq fx (nth 0 fr) fy (nth 1 fr)
          fw (nth 2 fr) fh (nth 3 fr)
          px (nth 1 placement) py (nth 2 placement)
          pw (nth 3 placement) ph (nth 4 placement)
          occW pw
          occH ph)

    ;; Пропил нужен только между соседними деталями.
    (if (< (+ px pw) (+ fx fw -1e-8))
      (setq occW (+ pw *CUTSHEET-WORK-KERF*)))
    (if (< (+ py ph) (+ fy fh -1e-8))
      (setq occH (+ ph *CUTSHEET-WORK-KERF*)))

    (setq newFree
      (append (cs-free-split-one fr px py occW occH)
              newFree)))
  (cs-prune-free newFree))

;; sheet = (freeRects placements usedBBoxArea usedActualArea)
(defun cs-new-sheet (w h)
  (list (list (cs-make-free-sheet w h)) '() 0.0 0.0))

(defun cs-sheet-placement (sheet part w h rotateFlag / p)
  (setq p (cs-find-placement-in-free (car sheet) w h rotateFlag))
  p)

(defun cs-sheet-add (sheet placement part / free placements newFree actual bboxArea)
  (setq newFree
    (cs-add-placement (car sheet) placement))
  ...
  (setq bboxArea (+ (nth 2 sheet)
                    (* (nth 3 part) (nth 4 part) (/ 1.0 1000000.0))))
  (setq actual (+ (nth 3 sheet) (nth 6 part)))
  (list newFree placements bboxArea actual))

(defun cs-nest (parts sheetW sheetH rotateFlag / sheets part best bestIdx bestP i sheet
                                         newSheet p placed oversized)
  (setq sheets '() oversized '()
        *CUTSHEET-WORK-KERF* *CUTSHEET-KERF*)
  (foreach part parts
    (setq best nil bestIdx nil bestP nil i 0)
    ;; Ищем лучший вариант во всех существующих листах.
    (foreach sheet sheets
      (setq p (cs-sheet-placement sheet part (nth 4 part) (nth 5 part) rotateFlag))
      (if p
        (if (or (null bestP)
                (< (car p) (car bestP))
                (and (= (car p) (car bestP))
                     (< (cadr p) (cadr bestP))))
          (setq bestP p bestIdx i)))
      (setq i (1+ i)))

    (if bestP
      (progn
        (setq sheet (nth bestIdx sheets))
        (setq sheet (cs-sheet-add sheet bestP part))
        (setq sheets (subst sheet (nth bestIdx sheets) sheets)))
      (progn
        ;; Новый лист.
        (setq newSheet (cs-new-sheet sheetW sheetH))
        (setq p (cs-sheet-placement newSheet part (nth 4 part) (nth 5 part) rotateFlag))
        (if p
          (setq sheets (append sheets (list (cs-sheet-add newSheet p part))))
          (setq oversized (cons part oversized))))))
  (list sheets (reverse oversized)))

;; ================= СТАТИСТИКА =================
(defun cs-total-count (records / n r)
  (setq n 0)
  (foreach r records (setq n (1+ n)))
  n)

(defun cs-total-actual-area-records (records / a r)
  (setq a 0.0)
  (foreach r records (setq a (+ a (nth 6 r))))
  a)

(defun cs-total-bbox-area-records (records / a r)
  (setq a 0.0)
  (foreach r records
    (setq a (+ a (* (nth 4 r) (nth 5 r) (/ 1.0 1000000.0)))))
  a)
...
(defun cs-ensure-italic-style (/ result)
  (if (tblsearch "STYLE" "Раскрой Italic")
    T
    (progn
      (setq result
        (entmake
...
(defun cs-ensure-bold-style (/ result)
  (if (tblsearch "STYLE" "Основной стиль (надписи без наклона)")
    T
    (progn
      (setq result
        (entmake
...
(defun cs-draw-filled-rect (p1 p2 color transparency / aci res ent)
  (setq aci (fix (+ 0.5 (* transparency 90.0))))
  (if (> aci 90) (setq aci 90))
  (if (< aci 0) (setq aci 0))
  (setq res
    (entmake
      (list '(0 . "SOLID") '(100 . "AcDbEntity") '(100 . "AcDbTrace")
            (cons 10 (list (car p1) (cadr p1) 0.0))
            (cons 11 (list (car p2) (cadr p1) 0.0))
            (cons 12 (list (car p1) (cadr p2) 0.0))
            (cons 13 (list (car p2) (cadr p2) 0.0))
            (cons 62 color))))
  (if res
    (progn
      (setq ent (entlast))
...
(defun cs-draw-text (pt h txt color / style)
  (setq style (if (tblsearch "STYLE" "Раскрой Italic")
                "Раскрой Italic" (getvar "TEXTSTYLE")))
  (entmake
...
(defun cs-draw-text-bold (pt h txt color / style)
  (setq style (if (tblsearch "STYLE" "Основной стиль (надписи без наклона)")
                "Основной стиль (надписи без наклона)"
                (getvar "TEXTSTYLE")))
  (entmake
    (list '(0 . "TEXT") '(100 . "AcDbEntity")
          (cons 62 color) (cons 7 style)
...
(defun cs-build-color-map (groups / out i g)
  (setq out '() i 0)
  (foreach g groups
    (setq out (cons (cons (car g)
                          (nth (rem i (length *CUTSHEET-PALETTE*))
                               *CUTSHEET-PALETTE*)) out)
          i (1+ i)))
  out)
...
(defun cs-draw-sheet-header (x y sheetW sheetH n / h textY)
  ...
  (cs-draw-text-bold (list x (+ y sheetH (* h 0.75)))
                     *CUTSHEET-TEXT-H*
                     (strcat "ЛИСТ " (itoa n)) *CUTSHEET-HEADER-COLOR*)
  (setq textY (+ y sheetH (* h 0.28)))
  (cs-draw-line (list x (+ y sheetH))
                (list (+ x sheetW) (+ y sheetH))
                *CUTSHEET-OUTLINE-COLOR*)
  (cs-draw-text (list x textY) (* *CUTSHEET-TEXT-H* 0.75)
                "0" *CUTSHEET-VALUE-COLOR*)
  (cs-draw-text (list (+ x sheetW -140.0) textY)
                (* *CUTSHEET-TEXT-H* 0.75)
                (cs-itoa-safe sheetW) *CUTSHEET-VALUE-COLOR*)
  (cs-draw-text (list (- x 240.0) (+ y sheetH -100.0))
                (* *CUTSHEET-TEXT-H* 0.75)
                (cs-itoa-safe sheetH) *CUTSHEET-VALUE-COLOR*))

(defun cs-draw-placement (pl x0 y0 colorMap / r px py w h rot
                             rw rh col txt actualBoxH)
  (setq r (car pl)
        px (+ x0 (nth 1 pl))
        py (+ y0 (nth 2 pl))
        w (nth 3 pl)
  ...
  (cs-draw-filled-rect (list px py) (list (+ px w) (+ py h))
                       col *CUTSHEET-PART-TRANSPARENCY*)
  (cs-draw-rect (list px py) (list (+ px w) (+ py h)) *CUTSHEET-OUTLINE-COLOR*)
  (if (> (* w h) 50000.0)
    (cs-draw-text-center
      (list (+ px (* w 0.5)) (+ py (* h 0.53)))
      (min *CUTSHEET-TEXT-H* (* 0.12 (min w h)))
      (cs-part-label r) *CUTSHEET-PART-TEXT-COLOR*))
  (if (= rot 1)
    (cs-draw-text-center
      (list (+ px (* w 0.5)) (+ py (* h 0.20)))
      (* *CUTSHEET-TEXT-H* 0.70) "90°" *CUTSHEET-PART-TEXT-COLOR*)))

(defun cs-draw-summary
       (groups sheets oversized sheetW sheetH insPt / left top width rowH
        rows y totalCnt actualArea bboxArea sheetArea kpdFact kpdBox
        waste sh p cnt rec size label colorMap rowTotal)
  (setq left (car insPt) top (cadr insPt)
        width *CUTSHEET-SUMMARY-W*
        rowH 220.0
        totalCnt 0 actualArea 0.0 bboxArea 0.0
        sheetArea (* (length sheets) sheetW sheetH (/ 1.0 1000000.0)))

  (foreach rec groups
    (setq totalCnt (+ totalCnt (nth 6 rec))
          bboxArea (+ bboxArea (nth 7 rec))
  ...
  ;; Рамка и заголовок
  (setq rows (length groups))
  (cs-draw-rect (list left (- top (* rowH (+ rows 9 (if oversized 1 0)))))
                (list (+ left width) top)
                *CUTSHEET-OUTLINE-COLOR*)
  (cs-draw-text-bold (list (+ left 100.0) (- top 0.72 rowH))
                     (* *CUTSHEET-TEXT-H* 1.45)
                     "РАСКРОЙ ЛИСТА" *CUTSHEET-TITLE-COLOR*)

  (setq y (- top (* rowH 2.0)))
  (cs-draw-text (list (+ left 100.0) y) *CUTSHEET-TEXT-H*
                (strcat "Лист: " (cs-itoa-safe sheetW) " x " (cs-itoa-safe sheetH) " мм")
                *CUTSHEET-HEADER-COLOR*)
  (setq y (- y rowH))
  (cs-draw-text (list (+ left 100.0) y) *CUTSHEET-TEXT-H*
                (strcat "Хлыстов/листов: " (itoa (length sheets)))
                *CUTSHEET-VALUE-COLOR*)
  (setq y (- y rowH))
  (cs-draw-text (list (+ left 100.0) y) *CUTSHEET-TEXT-H*
                (strcat "Всего деталей: " (itoa totalCnt) " шт.")
                *CUTSHEET-VALUE-COLOR*)
  (setq y (- y rowH))
  (cs-draw-text (list (+ left 100.0) y) *CUTSHEET-TEXT-H*
                (strcat "Фактическая площадь: " (cs-format-num actualArea 2) " м2")
                *CUTSHEET-VALUE-COLOR*)
  (setq y (- y rowH))
  (cs-draw-text (list (+ left 100.0) y) *CUTSHEET-TEXT-H*
                (strcat "Площадь габаритов: " (cs-format-num bboxArea 2) " м2")
                *CUTSHEET-VALUE-COLOR*)
  (setq y (- y rowH))
  (cs-draw-text-bold (list (+ left 100.0) y) *CUTSHEET-TEXT-H*
                (strcat "ПОЛЕЗНЫЙ ВЫХОД: " (cs-format-num kpdFact 1) "%")
                *CUTSHEET-KPD-COLOR*)
  (setq y (- y rowH))
  (cs-draw-text (list (+ left 100.0) y) *CUTSHEET-TEXT-H*
                (strcat "Габаритный выход: " (cs-format-num kpdBox 1) "%")
                *CUTSHEET-VALUE-COLOR*)
  (setq y (- y rowH))
  (cs-draw-text (list (+ left 100.0) y) *CUTSHEET-TEXT-H*
                (strcat "Потери/остаток по площади: " (cs-format-num waste 2) " м2")
                *CUTSHEET-WASTE-COLOR*)

  (setq y (- y (* 1.2 rowH)))
  (cs-draw-text-bold (list (+ left 100.0) y) *CUTSHEET-TEXT-H*
                     "ПОЗИЦИИ" *CUTSHEET-TITLE-COLOR*)
  (setq y (- y rowH))
  (cs-draw-text (list (+ left 100.0) y) (* *CUTSHEET-TEXT-H* 0.82)
                "Размер" *CUTSHEET-HEADER-COLOR*)
  (cs-draw-text (list (+ left 1200.0) y) (* *CUTSHEET-TEXT-H* 0.82)
                "Кол-во" *CUTSHEET-HEADER-COLOR*)
  (cs-draw-text (list (+ left 1700.0) y) (* *CUTSHEET-TEXT-H* 0.82)
                "Площадь" *CUTSHEET-HEADER-COLOR*)
  (setq y (- y rowH))
  (setq colorMap (cs-build-color-map groups))
  (foreach rec groups
    (setq label (cs-part-label
                  (list nil (nth 1 rec) (nth 2 rec) (nth 3 rec)
                        (nth 4 rec) (nth 5 rec) (nth 6 rec)
                        nil nil nil))
          col (cs-color-for-part
                (list nil (nth 1 rec) (nth 2 rec) (nth 3 rec)
                      (nth 4 rec) (nth 5 rec) (nth 6 rec)
                      nil nil nil)
                colorMap))
    (cs-draw-text (list (+ left 100.0) y)
                  (* *CUTSHEET-TEXT-H* 0.82) label col)
    (cs-draw-text (list (+ left 1200.0) y)
                  (* *CUTSHEET-TEXT-H* 0.82) (itoa (nth 6 rec)) *CUTSHEET-VALUE-COLOR*)
    (cs-draw-text (list (+ left 1700.0) y)
  ...
  (if oversized
    (progn
      (setq y (- y rowH))
      (cs-draw-text-bold (list (+ left 100.0) y) *CUTSHEET-TEXT-H*
                         (strcat "НЕРАЗМЕЩЕНО: " (itoa (length oversized)) " шт.")
                         *CUTSHEET-WASTE-COLOR*)
    ))
  (list (list left (- top (* rowH (+ rows 9 (if oversized 1 0)))))
        (list (+ left width) top)))
...
(defun cs-draw-layout (sheets sheetW sheetH insPt colorMap / n sh col row x y p)
  ...
  (foreach sh sheets
    (setq n (1+ n)
          col (rem (1- n) *CUTSHEET-GRID-COLS*)
          row (fix (/ (1- n) *CUTSHEET-GRID-COLS*))
          x (+ (car insPt) (* col (+ sheetW *CUTSHEET-SHEET-GAP*)))
          y (- (cadr insPt)
               (* row (+ sheetH *CUTSHEET-SHEET-GAP* *CUTSHEET-SHEET-HEADER*))))
    (cs-draw-sheet-header x y sheetW sheetH n)
    (cs-draw-rect (list x y) (list (+ x sheetW) (+ y sheetH)) *CUTSHEET-OUTLINE-COLOR*)
    ;; Остаточная площадь без отдельной геометрии отхода: 
    ;; свободное пространство остаётся видимым в листе.
    (foreach p (cadr sh)
      (cs-draw-placement p x y colorMap)))
  (list (list (car insPt)
              (- (cadr insPt)
                 (* (max 0 (fix (/ (max 0 (1- n)) *CUTSHEET-GRID-COLS*)))
                    (+ sheetH *CUTSHEET-SHEET-GAP* *CUTSHEET-SHEET-HEADER*)))
        )
        (list (+ (car insPt)
                 (* (max 0 (1- (min n *CUTSHEET-GRID-COLS*)))
                    (+ sheetW *CUTSHEET-SHEET-GAP*))
                 sheetW)
              (+ (cadr insPt) sheetH))))
...
(defun cs-write-csv (records sheets oversized sheetW sheetH kerf / fname f n sh p r)
        ...
        (foreach p (cadr sh)
          ...
          (write-line
            (strcat (itoa n) ";"
                    ...
                    (nth 3 r) ";"
                    (cs-part-label r) ";"
                    (cs-format-num (nth 1 p) 1) ";"
                    (cs-format-num (nth 2 p) 1) ";"
                    (if (= (nth 5 p) 1) "90" "0") ";"
                    (cs-format-num (nth 6 r) 4))
            f)))
      (if oversized
        (progn
          (write-line "" f)
          ...
          (foreach r oversized
            (write-line
              (strcat (itoa (car r)) ";" (nth 3 r) ";" (cs-part-label r)
...
(defun cs-write-xls (records groups sheets oversized sheetW sheetH kerf / fname f n sh p r rec totalCnt actualArea bboxArea sheetArea kpdFact kpdBox)
  (setq fname (strcat (getvar "DWGPREFIX")
                      (vl-filename-base (getvar "DWGNAME"))
                      " Раскрой листа.xls"))
  (setq f (open fname "w"))
  (if f
    (progn
      ...
      (write-line "<Styles>" f)
      (write-line "<Style ss:ID=\"T\"><Font ss:Bold=\"1\" ss:Size=\"13\"/><Interior ss:Color=\"#D9D9D9\" ss:Pattern=\"Solid\"/></Style>" f)
      (write-line "<Style ss:ID=\"H\"><Font ss:Bold=\"1\"/><Interior ss:Color=\"#D9D9D9\" ss:Pattern=\"Solid\"/></Style>" f)
      (write-line "</Styles>" f)
      ...
      (write-line (strcat "<Row><Cell><Data ss:Type=\"String\">Формат листа</Data></Cell><Cell><Data ss:Type=\"String\">" (cs-itoa-safe sheetW) "x" (cs-itoa-safe sheetH) " мм</Data></Cell></Row>") f)
      (write-line (strcat "<Row><Cell><Data ss:Type=\"String\">Пропил</Data></Cell><Cell><Data ss:Type=\"Number\">" (rtos kerf 2 2) "</Data></Cell></Row>") f)
      (write-line (strcat "<Row><Cell><Data ss:Type=\"String\">Листов</Data></Cell><Cell><Data ss:Type=\"Number\">" (itoa (length sheets)) "</Data></Cell></Row>") f)
      (write-line (strcat "<Row><Cell><Data ss:Type=\"String\">Деталей</Data></Cell><Cell><Data ss:Type=\"Number\">" (itoa totalCnt) "</Data></Cell></Row>") f)
      (write-line (strcat "<Row><Cell><Data ss:Type=\"String\">Фактическая площадь, м2</Data></Cell><Cell><Data ss:Type=\"Number\">" (rtos actualArea 2 4) "</Data></Cell></Row>") f)
      (write-line (strcat "<Row><Cell><Data ss:Type=\"String\">Площадь габаритов, м2</Data></Cell><Cell><Data ss:Type=\"Number\">" (rtos bboxArea 2 4) "</Data></Cell></Row>") f)
      (write-line (strcat "<Row><Cell ss:StyleID=\"H\"><Data ss:Type=\"String\">Полезный выход, %</Data></Cell><Cell ss:StyleID=\"H\"><Data ss:Type=\"Number\">" (rtos kpdFact 2 1) "</Data></Cell></Row>") f)
      (write-line (strcat "<Row><Cell><Data ss:Type=\"String\">Габаритный выход, %</Data></Cell><Cell><Data ss:Type=\"Number\">" (rtos kpdBox 2 1) "</Data></Cell></Row>") f)
      ...
      (foreach rec groups
        (write-line
          (strcat "<Row><Cell><Data ss:Type=\"String\">"
                  (cs-part-label (list nil (nth 1 rec) (nth 2 rec) (nth 3 rec) (nth 4 rec) (nth 5 rec) (nth 6 rec) nil nil nil))
                  "</Data></Cell><Cell><Data ss:Type=\"String\">"
                  (cs-xml-escape (nth 3 rec))
      ...
      (foreach sh sheets
        ...
        (foreach p (cadr sh)
          (setq r (car p))
          (write-line
            (strcat "<Row><Cell><Data ss:Type=\"Number\">" (itoa n) "</Data></Cell>"
                    "<Cell><Data ss:Type=\"Number\">" (itoa (car r)) "</Data></Cell>"
                    "<Cell><Data ss:Type=\"String\">" (cs-xml-escape (nth 3 r)) "</Data></Cell>"
                    "<Cell><Data ss:Type=\"String\">" (cs-part-label r) "</Data></Cell>"
                    "<Cell><Data ss:Type=\"Number\">" (rtos (nth 1 p) 2 1) "</Data></Cell>"
                    "<Cell><Data ss:Type=\"Number\">" (rtos (nth 2 p) 2 1) "</Data></Cell>"
                    "<Cell><Data ss:Type=\"Number\">" (if (= (nth 5 p) 1) "90" "0") "</Data></Cell>"
                    "<Cell><Data ss:Type=\"Number\">" (rtos (nth 6 r) 2 4) "</Data></Cell></Row>")
            f)))
      (write-line "</Table></Worksheet>" f)
      ...
      (if oversized
        (progn
          ...
          (foreach r oversized
            (write-line
              (strcat "<Row><Cell><Data ss:Type=\"Number\">" (itoa (car r)) "</Data></Cell>"
                      "<Cell><Data ss:Type=\"String\">" (cs-xml-escape (nth 3 r)) "</Data></Cell>"
                      "<Cell><Data ss:Type=\"String\">" (cs-part-label r) "</Data></Cell>"
                      "<Cell><Data ss:Type=\"Number\">" (rtos (nth 6 r) 2 4) "</Data></Cell></Row>")
              f))
...
;; ================= ГЛАВНАЯ ФУНКЦИЯ =================
(defun cutsheet-main (layers-from-caller / *error*
                      ss polyCnt dynCnt dynTypes
  ...
  (defun *error* (msg)
    ...
    (if ftrans (cs-transparency-restore ftrans))
    (princ))

  (princ "\n=== РАСКРОЙ ЛИСТА ===")

  ;; Автономный запуск: разрешаем ручной ввод слоёв, как в CUTLINE.
  ...
  ;; Сбор исходного набора один раз — важен для совместимости с preselection.
  ...
  (princ (strcat "\nПолилиний: " (itoa polyCnt)
                 ", динамических блоков: " (itoa dynCnt)))

  (if (= (+ polyCnt dynCnt) 0)
    (progn (princ "\nПодходящих исходных объектов нет.") (princ) (exit)))

  ;; Значения из dispatcher / последнего запуска.
  ...
  (if (boundp '*CUTSHEET-CREATE-XLS*)
    (setq defaultXls *CUTSHEET-CREATE-XLS*))
  (if (boundp '*CUTSHEET-CREATE-TABLE*)
    (setq defaultAcad *CUTSHEET-CREATE-TABLE*))

  (setq r (cs-dialog polyCnt dynCnt dynTypes ss
                     defaultW defaultH defaultKerf defaultRotate
                     defaultXls defaultAcad))
  (if (null r)
  ...
  (setq *CUTSHEET-LAST-WIDTH* sheetW
        *CUTSHEET-LAST-HEIGHT* sheetH
        *CUTSHEET-LAST-KERF* kerf
        *CUTSHEET-LAST-ROTATE* rotateFlag
        *CUTSHEET-LAST-XLS* exportXls
        *CUTSHEET-LAST-ACAD* exportAcad)
  ...
  ;; Предварительные сообщения по непрямоугольной геометрии.
  (princ (strcat "\nВ раскрой принято деталей: " (itoa (length records))))

  (setq groups (cs-aggregate records)
        parts (cs-expanded-sorted records))

  (setq nested (cs-nest parts sheetW sheetH rotateFlag)
        sheets (car nested)
  ...
  (princ (strcat "\nЛистов: " (itoa (length sheets))
                 " | деталей: " (itoa totalCnt)
  ...
  ;; XLS
  (if exportXls
    (if (not (cs-write-xls records groups sheets oversized sheetW sheetH kerf))
      (progn
        (princ "\nXLS не создан — выполняется fallback CSV.")
        (cs-write-csv records sheets oversized sheetW sheetH kerf))))
    ...
    (progn
      ...
      (if insPt
        (progn
          ...
          (n1-cs-setvar-transparency-display)

          (setq bbox1 (cs-draw-layout sheets sheetW sheetH insPt colorMap))
          (setq bbox2
            (cs-draw-summary groups sheets oversized sheetW sheetH
          ...
          ;; Объединяем свежую геометрию в блок карты.
          (setq ssNew (ssadd)
                ent (if lastEnt (entnext lastEnt) (entnext)))
          (while ent
            (ssadd ent ssNew)
          ...
          (if (> (sslength ssNew) 0)
            (progn
              (setq baseName (vl-filename-base (getvar "DWGNAME"))
                    blockName (strcat "Раскрой листа " baseName))
              ...
              (if (tblsearch "BLOCK" blockName)
                (princ (strcat "\nСоздан блок карты раскроя: " blockName))))

          (if (and doc uMark)
            (progn (vla-EndUndoMark doc) (setq uMark nil)))
  ...
  (if (and app bbox)
    (progn
      (setq p1 (vlax-3d-point
                  (list (car (car bbox)) (cadr (car bbox)) 0.0))
            p2 (vlax-3d-point
                  (list (car (cadr bbox)) (cadr (cadr bbox)) 0.0)))
      (vl-catch-all-apply 'vla-ZoomWindow (list app p1 p2)))))

(defun n1-cs-setvar-transparency-display ()
  (vl-catch-all-apply 'setvar (list "TRANSPARENCYDISPLAY" 1)))

(defun cs-transparency-restore (x) x)

;; ================= КОМАНДЫ =================
(defun c:CUTSHEET ()
  (cutsheet-main 'ASK))

(defun c:РАСКРОЙЛИСТА ()
  (cutsheet-main 'ASK))

(princ "\nCUTSHEET.LSP загружен. Команды: CUTSHEET, РАСКРОЙЛИСТА")
(princ)
