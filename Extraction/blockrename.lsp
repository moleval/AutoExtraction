...
;;;     остается видимым до закрытия диспетчера
...
;; Остальные — с учетом чекбокса и поиска.
;; ============================================================
    ...
    (T
     ...
     (if (null insPt)
       ...
       (progn
         ...
         ;; Экспортируем блок во временный файл через command (надежнее для имен с пробелами)
         (vl-catch-all-apply
