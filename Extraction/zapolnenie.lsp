...
(defun zapolnenie-aggregate
    ...
    (if dims
      ...
      (progn
        ...
        (if found
          ...
          ;;   5 = count
          ;;
          ;; Поэтому количество необходимо брать из:
          ;;   (car (cddddr found))
          ;; ==================================================
  ...
  ;; ==========================================================
  ;; Удаляем внутренний ключ ДО сортировки: zapolnenie-sort-less
  ;; разбирает запись как (name h w count). Если сортировать записи
  ;; с ключом, (car a) = key и порядок становится СТРОКОВЫМ
  ;; («1226» перед «526») — регрессия бага №2.
  (vl-sort
    (mapcar
      '(lambda (r)
         (list
...
(defun c:zapolnenie
  ...
  (if (= layers-str "")
    ...
    ;; Заполнение, Стекло
    ;;
    ;; второй элемент становился:
    ;;
    ;; " СТЕКЛО"
    ;;
    ;; т.к. split-string не удаляет пробелы.
    ;;
    ;; Теперь каждый элемент сначала очищается:
    ;;
    ;; vl-string-trim " "
    ;;
    ;; ========================================================
