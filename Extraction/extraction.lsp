...
;;;
;;; ÄÎÁÀÂËÅÍÎ (Á4): â âåòêå CLADDING ôóíêöèè run-task ïåðåäà¸òñÿ
;;; ñåäüìûì àðãóìåíòîì T — äèñïåò÷åð ñîáèðàåò è ïîëèëèíèè, è áëîêè.
;;; ============================================================
...
;; ============================================================
;; Ñëîè ïî óìîë÷àíèþ äëÿ êàæäîé çàäà÷è
;; ============================================================

(if (not (boundp '*EXTRACTION-LAST-FASONKA-LAYERS*))
  (setq *EXTRACTION-LAST-FASONKA-LAYERS* '("Ôàñîíêà*" "Æåëåçî*"))
)

(if (not (boundp '*EXTRACTION-LAST-ZAPOLNENIE-LAYERS*))
  (setq *EXTRACTION-LAST-ZAPOLNENIE-LAYERS* '("Çàïîëíåíèå" "Ñòåêëî" "Îáîçíà÷åíèå ñò-ò"))
)

(if (not (boundp '*EXTRACTION-LAST-CLADDING-LAYERS*))
  (setq *EXTRACTION-LAST-CLADDING-LAYERS* '("*Îáëèöîâêà*" "*Êàññåò*" "*Êåðàìîãðàíèò*"))
)

(if (not (boundp '*EXTRACTION-LAST-VITRAZH-LAYERS*))
  (setq *EXTRACTION-LAST-VITRAZH-LAYERS* '("Âèòðàæè" "Ñòîéê*" "Ðèãåë*"))
)
...
;; ============================================================
;; Î×ÈÑÒÊÀ ÑÏÈÑÊÀ ÑËÎ¨Â ÄËß ÐÀÑÊÐÎß
;; ============================================================
...
;; ============================================================
;; ÎÁÐÀÁÎÒÊÀ ÐÅÇÓËÜÒÀÒÀ ÂÛÇÎÂÀ ÌÎÄÓËß
;; ============================================================
  ...
  (if (vl-catch-all-error-p r)
    (progn
      ...
      (if (or
            ...
            (vl-string-search "ÎÒÌÅÍ" errMsgUp)
            (vl-string-search "ÇÀÂÅÐØÈÒÜ" errMsgUp)
            (vl-string-search "ÏÐÅÐÂÀÒÜ" errMsgUp)
            (vl-string-search "ÂÛÉÒÈ" errMsgUp))
        nil
        (princ (strcat "\nÌîäóëü " module-name
                       " íå çàãðóæåí èëè îøèáêà âûïîëíåíèÿ: "
                       errMsg))
      )
    )
  )
...
;; ============================================================
;; ÏÅÐÅÊËÞ×ÅÍÈÅ ÇÀÄÀ×È È ÂÎÑÑÒÀÍÎÂËÅÍÈÅ ÑËÎ¨Â
;; ============================================================
...
;; ============================================================
;; ÇÀÏÓÑÊ ÇÀÄÀ×È
;; ÄÎÁÀÂËÅÍÎ (Á4): â âåòêå CLADDING ïåðåäà¸òñÿ T ñåäüìûì
;; àðãóìåíòîì — äèñïåò÷åð ñîáèðàåò è ïîëèëèíèè, è áëîêè.
;; ============================================================
(defun run-task
  ...
  (cond
    ((eq task-id 'FASONKA)
     ...
     (extraction-handle-module-result r "Ôàñîíêà")
    )
    ...
    ((eq task-id 'SUBSYSTEM)
     (setq r
       (vl-catch-all-apply 'subsystem-main
         (list layers report-mode export-excel export-txt
               create-table save-base)))
     (extraction-handle-module-result r "Ïîäñèñòåìà")
    )

    ;; ÄÎÁÀÂËÅÍÎ (Á4): T = äåëàòü áëîêè âñëåä çà ïîëèëèíèÿìè
    ((eq task-id 'CLADDING)
     (setq r
       (vl-catch-all-apply 'cladding-main
         (list layers report-mode export-excel export-txt
               create-table save-base T)))
     (extraction-handle-module-result r "Îáëèöîâêà")
    )
    ...
    ((eq task-id 'VITRAZH)
     (setq r
       (vl-catch-all-apply 'vitrazh-main
         (list layers report-mode export-excel export-txt
               create-table save-base)))
     (extraction-handle-module-result r "Âèòðàæ")
    )
    ...
    ((eq task-id 'ZAPOLNENIE)
     (setq r
       (vl-catch-all-apply 'zapolnenie-main
         (list layers report-mode export-excel export-txt
               create-table save-base)))
     (extraction-handle-module-result r "Çàïîëíåíèå")
    )
...
(defun extraction-help ()
  (alert
    (strcat
      ...
      "Ñëîè ïî óìîë÷àíèþ âûäåëÿþòñÿ àâòîìàòè÷åñêè "
      "ïðè âûáîðå çàäà÷è.\n"
      "Èçìåí¸ííûé âûáîð ñîõðàíÿåòñÿ äëÿ êàæäîãî "
      "çàäà÷è îòäåëüíî.\n\n"
